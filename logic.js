...
function filterZero(tiles) {
	// create ne array removing the zeroes
...
function slideLeft() {
	for(let r = 0; r < rows; r++) {
		...
		// slide() function will retuen a new value for a specific row. (merging of tiles)
		row = slide(row);
      ...
      // line for animation
      // if currrent tile != to the original tile, apply animation
...
function slideRight() {
	for(let r = 0; r < rows; r++) {
		...
		// reverse the array row since it is sliding to right
		// r = 0: [0,2,2,2] -> [2,2,2,0]
		row.reverse();
...
function hasEmptyTile() {
	for (let r = 0; r < rows; r++) {
		for (let c = 0; c < columns; c++) {
			// Check if current tile == 0, iff yes it will return true
			if(board[r][c] == 0) {
				return true;
			}
		}
...
function hasLost() {
	...
	{
	// Found adjacemnt cells with the same value
	return false;
		}
	}
}
...
// event listener to capture touch in the scree and assigns the x and y coordinates in the startX and startY variable.
document.addEventListener("touchstart", (e) => {
	startX = e.touches[0].clientX;
	startY = e.touches[0].clientY;
})

// event listener to check where you touch the screen and prevents scrolling. input targets any elements that includes the word tile.
